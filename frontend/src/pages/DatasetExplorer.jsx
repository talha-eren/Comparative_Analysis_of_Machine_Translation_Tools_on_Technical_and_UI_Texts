...
function DatasetExplorer() {
    ...
    return (
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="card text-center py-12">
          <p className="text-xl text-gray-600 mb-4">Dataset bulunamadı</p>
          <p className="text-gray-500">Önce dataset\'leri indirin</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <h1 className="text-3xl font-bold mb-8">Dataset Gezgini</h1>
              ...
              {stats && (
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between">
                    <span className="text-gray-600">Toplam Segment:</span>
                    <span className="font-semibold">{stats.total_segments?.toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-600">Ortalama Uzunluk:</span>
                    <span className="font-semibold">{stats.avg_length?.toFixed(1)} char</span>
                  </div>
                  
                  {stats.categories && (
                    <div className="mt-3 pt-3 border-t">
                      <p className="text-gray-600 mb-2">Kategoriler:</p>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {Object.entries(datasets.statistics).map(([name, stats]) => (
                  <tr key={name} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      {stats.avg_length?.toFixed(1)} char
                    </td>
